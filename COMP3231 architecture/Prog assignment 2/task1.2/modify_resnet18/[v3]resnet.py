...
class Stage3(torch.nn.Module):
    def __init__(self):
        ...
        self.layer45a = torch.nn.ReLU(inplace=True)
        self.layer46a = torch.nn.Conv2d(1024, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        self.layer47a = torch.nn.BatchNorm2d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.layer48a = torch.nn.ReLU(inplace=True)
        self.layer49a = torch.nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        self.layer50a = torch.nn.BatchNorm2d(256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.layer51a = torch.nn.ReLU(inplace=True)
        self.layer52a = torch.nn.Conv2d(256, 1024, kernel_size=(1, 1), stride=(1, 1), bias=False)
        self.layer53a = torch.nn.BatchNorm2d(1024, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
    ...
    def forward(self, x_rref):
        ...
        with self._lock:
            ...
            out45a = self.layer45a(x)
            out46a = self.layer46a(out45a)
            out47a = self.layer47a(out46a)
            out48a = self.layer48a(out47a)
            out49a = self.layer49a(out48a)
            out50a = self.layer50a(out49a)
            out51a = self.layer51a(out50a)
            out52a = self.layer52a(out51a)
            out53a = self.layer53a(out52a)
            out54a = out53a+out45a
            
            out4 = self.layer4(out54a)
            out5 = self.layer5(out4)
            out6 = self.layer6(out5)
            out7 = self.layer7(out6)
            out8 = self.layer8(out7)
